...
struct HashTableGrower {
    ...
    u8 size_degree = initial_size_degree;
...
template <bool enable, typename Cell>
struct AllocatorBufferDeleter;

template <typename Cell>
struct AllocatorBufferDeleter<false, Cell> {
    AllocatorBufferDeleter(size_t) {}

    void operator()(Cell *) const {}
};

template <typename Cell>
struct AllocatorBufferDeleter<true, Cell> {
    AllocatorBufferDeleter(size_t size_) : size(size_) {}

    void operator()(Cell *buffer) const { mock_std::free(buffer); }
...
protected:
    ...
    size_t inline findCell(const Key &x, size_t hash_value, size_t place_value) const {
    ...
    size_t inline findEmptyCell(size_t place_value) const {
    ...
    void alloc(const Grower &new_grower) {
        buf = reinterpret_cast<Cell *>(mock_std::malloc(new_grower.bufSize() * sizeof(Cell)));
        memset(reinterpret_cast<char *>(buf), 0, new_grower.bufSize() * sizeof(Cell));
        grower = new_grower;
    }
    ...
    void resize(size_t for_num_elems = 0, size_t for_buf_size = 0) {
         ...
         */
        using Deleter = AllocatorBufferDeleter<Cell::need_to_notify_cell_during_move, Cell>;
        Deleter buffer_deleter(old_buffer_size);
        ...
        if constexpr (Cell::need_to_notify_cell_during_move) {
            ...
            memset(reinterpret_cast<char *>(buf) + old_buffer_size, 0, new_grower.bufSize() * sizeof(Cell) - old_buffer_size);
        } else {
            buf = reinterpret_cast<Cell *>(mock_std::realloc(buf, new_grower.bufSize() * sizeof(Cell)));
            memset(reinterpret_cast<char *>(buf) + old_buffer_size, 0, new_grower.bufSize() * sizeof(Cell) - old_buffer_size);
        }
...
protected:
    ...
    bool inline emplaceIfZero(const Key &x, LookupResult &it, bool &inserted, size_t hash_value) {
    ...
    void inline emplaceNonZeroImpl(size_t place_value, const Key key,
    ...
    void inline emplaceNonZero(const Key key, LookupResult &it,
                               bool &inserted, size_t hash_value) {
        size_t place_value = findCell(key, hash_value, grower.place(hash_value));
        emplaceNonZeroImpl(place_value, key, it, inserted, hash_value);
    }
...
public:
    ...
    std::pair<LookupResult, bool> inline insert(const value_type &x) {
    ...
    void inline reinsert(iterator &it, size_t hash_value) {
     ...
     */
    void inline emplace(const Key key, LookupResult &it, bool &inserted) {
        emplace(key, it, inserted, hash(key));
    }

    void inline emplace(const Key key, LookupResult &it,
                        bool &inserted, size_t hash_value) {
        if (!emplaceIfZero(key, it, inserted, hash_value))
            emplaceNonZero(key, it, inserted, hash_value);
    }
    ...
    void inline insertUniqueNonZero(const Cell *cell, size_t hash_value) {
    ...
    LookupResult inline find(const Key &x) {
        if (Cell::isZero(x, *this))
    ...
    ConstLookupResult inline find(const Key &x) const {
        return const_cast<std::decay_t<decltype(*this)> *>(this)->find(x);
    }

    LookupResult inline find(const Key &x, size_t hash_value) {
        if (Cell::isZero(x, *this))
            return this->hasZero() ? this->zeroValue() : nullptr;
    ...
    ConstLookupResult inline find(const Key &x, size_t hash_value) const {
        return const_cast<std::decay_t<decltype(*this)> *>(this)->find(x, hash_value);
    }

    inline bool erase(const Key &x) requires Grower::performs_linear_probing_with_single_step {
        return erase(x, hash(x));
    }

    inline bool erase(const Key &x, size_t hash_value) requires Grower::performs_linear_probing_with_single_step {
    ...
    bool inline has(const Key &x) const {
        if (Cell::isZero(x, *this))
            return this->hasZero();
    ...
    bool inline has(const Key &x, size_t hash_value) const {
        if (Cell::isZero(x, *this))
            return this->hasZero();
