...
use hashtable::twolevel_hashtable::TwolevelHashtable;
use rand::Rng;
use std::collections::HashSet;
use std::time::{Duration, Instant};

#[test]
fn count_distinct_twolevel() {
    let mut sequence = vec![0u64; 1 << 26];
    sequence.fill_with(|| rand::thread_rng().gen_range(0..1 << 30));
    let mut hashtable = TwoLevelHashMap::<u64, u64>::create();
    let mut other = TwolevelHashtable::<u64, u64>::new();
    std::thread::scope(|scope| {
        ...
        scope.spawn(|| {
            ...
            let end = Instant::now();
            println!("time_twolevel = {:?}", end - start);
        });
    });
